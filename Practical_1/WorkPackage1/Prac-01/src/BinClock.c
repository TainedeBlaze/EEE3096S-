...
int main(void){
	...
	//wiringPiI2CWriteReg8(RTC, SEC_REGISTER, 0xcollect2: error: ld returned 1 exit statu
...
void hourInc(void){
	...
	int hourstemp = wiringPiI2CReadReg8(RTC, HOUR_REGISTER); 
	hourstemp= hexCompensation(hourstemp) ; 
        if(hourstemp < 23){
         hourstemp++;
        }
        else{
        hourstemp =0; }
        hours =wiringPiI2CWriteReg8(RTC, HOUR_REGISTER, decCompensation(hourstemp));
